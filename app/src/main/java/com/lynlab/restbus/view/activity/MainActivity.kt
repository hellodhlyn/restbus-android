...
class MainActivity : AppCompatActivity() {
    ...
    private fun setViewPager() {
        val viewPager = findViewById(R.id.activity_main_viewpager) as LockableViewPager
        val nextPageSubject : PublishSubject<Any?> = PublishSubject.create()
        nextPageSubject.subscribe({ viewPager.currentItem = viewPager.currentItem + 1 })

        val adapter = MainViewPagerAdapter(supportFragmentManager)
        adapter.addItem(SearchRouteFragment(nextPageSubject))
        adapter.addItem(StationListFragment(nextPageSubject))
